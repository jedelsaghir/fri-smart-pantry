...
import type { CatalogItem, CatalogMergeGroup, StorageKey } from "@/types/pantry";
...
export function ItemDatabaseSection({
  ...
  onAdd: (input: { name: string; unit: string; emoji: string; defaultStorage: StorageKey }) => void;
  ...
  const [defaultStorage, setDefaultStorage] = useState<StorageKey>("fridge");
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editDraft, setEditDraft] = useState({
    name: "",
    unit: "",
    emoji: "",
    defaultStorage: "fridge" as StorageKey,
  });
  ...
  const startEdit = (item: CatalogItem) => {
    ...
    setEditDraft({
      ...
      unit: item.unit,
      emoji: item.emoji,
      defaultStorage: item.defaultStorage ?? "fridge",
    });
  };
  ...
  const saveEdit = () => {
    ...
    onUpdate(editingId, {
      ...
      defaultStorage: editDraft.defaultStorage,
    });
    setEditingId(null);
  };
        ...
        <div className="elevated-card mb-3 space-y-2.5 rounded-3xl p-4">
          ...
          <div>
            <p className="mb-1.5 text-[11px] font-medium text-muted-foreground">
              Default location <span className="text-destructive">*</span>
            </p>
            <div className="grid grid-cols-3 gap-1.5" role="group" aria-label="Default storage">
              {(
                [
                  ["fridge", "Fridge"],
                  ["freezer", "Freezer"],
                ...
                const active = defaultStorage === key;
                return (
                  <button
                    key={key}
                    type="button"
                    onClick={() => setDefaultStorage(key)}
                    className={
                      "min-h-11 rounded-2xl border text-sm font-semibold transition active:scale-[0.98] " +
                      (active
            ...
            <p className="mt-1.5 text-[11px] text-muted-foreground leading-snug">
              Used when this item is found on a receipt or added from the list.
            </p>
          </div>
          <div className="flex gap-2">
            <button
            ...
            <button
              ...
              onClick={submitAdd}
              disabled={!name.trim()}
              className="flex-1 rounded-2xl bg-brand py-2.5 text-sm font-semibold text-brand-foreground active:scale-[0.985] disabled:opacity-50"
            >
              Save item
            </button>
          </div>
                ...
                <div className="flex w-full flex-col gap-2">
                  ...
                  <div className="grid grid-cols-3 gap-1.5" role="group" aria-label="Default storage">
                    {(
                      ...
                      const active = editDraft.defaultStorage === key;
                      return (
                        <button
                          key={key}
                          type="button"
                          onClick={() =>
                            setEditDraft((d) => ({ ...d, defaultStorage: key }))
                          }
                          className={
                            "min-h-10 rounded-xl border text-xs font-semibold transition active:scale-[0.98] " +
                            (active
                              ? "border-brand/40 bg-brand text-brand-foreground"
                              : "border-border/50 bg-secondary/50 text-foreground active:bg-secondary")
                          }
                ...
                <>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {item.unit}
                      {item.defaultStorage
                        ? ` · ${item.defaultStorage === "fridge" ? "Fridge" : item.defaultStorage === "freezer" ? "Freezer" : "Pantry"}`
                        : ""}
