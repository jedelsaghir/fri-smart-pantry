...
export function ReceiptReviewFooter({
  ...
  return (
    <div className="shrink-0 border-t border-border/60 bg-background/95 backdrop-blur-md px-5 pt-4 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-[0_-8px_24px_-12px_oklch(0.2_0.02_150_/_0.12)]">
      <button
        type="button"
        onClick={onConfirm}
        disabled={reviewCount === 0}
        className="touch-target w-full min-h-12 rounded-3xl bg-brand py-4 text-lg font-semibold text-brand-foreground active:scale-[0.985] disabled:opacity-60 transition disabled:active:scale-100 flex items-center justify-center gap-2"
      >
      ...
      <button
        type="button"
        onClick={onSkip}
        className="touch-target mt-2.5 w-full min-h-11 py-2.5 text-sm font-medium text-muted-foreground active:text-foreground"
      >
        Skip for now
      </button>
    </div>
