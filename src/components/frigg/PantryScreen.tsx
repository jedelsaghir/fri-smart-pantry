...
export function PantryScreen() {
  ...
  const [addSheetBarcode, setAddSheetBarcode] = useState(false);
  ...
  const handleAddToPantry = useCallback(
    (input: {
      ...
      const newItem = {
        ...
        unit: input.unit,
        emoji: input.emoji,
        daysLeft: input.daysLeft === undefined ? null : input.daysLeft,
      ...
      rememberPantryItem({ ...newItem, defaultStorage: active }, "pantry_add");
  ...
  return (
    <div className="relative min-h-dvh pb-32 bg-background touch-manipulation">
      ...
      {!isListView &&
        ...
        !searchingAll && (
          <ScanFab
            onReceiptScan={() => setScanOpen(true)}
            onBarcodeScan={() => {
              setAddSheetBarcode(true);
      ...
      <PantryAddSheet
        ...
        onOpenChange={(v) => {
          setAddSheetOpen(v);
          if (!v) setAddSheetBarcode(false);
        }}
        storage={active}
        suggest={suggest}
        onAdd={handleAddToPantry}
        autoOpenBarcode={addSheetBarcode}
        pantryItems={[
          ...items.fridge.map((i) => ({
            id: i.id,
            name: i.name,
