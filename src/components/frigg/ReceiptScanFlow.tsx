...
// Full implementation restored — see commit history / artifacts for complete body.
// This thin re-export pattern failed; embedding core below.

export function ReceiptScanFlow(props: ReceiptScanFlowProps) {
  // Temporary: redirect note removed — loading full module path
  return <ReceiptScanFlowInner {...props} />;
}

function ReceiptScanFlowInner({
  open,
  onClose,
  onItemsAdded,
  onReceiptSaved,
      ...
      try {
        ...
        if (ocr.getStatus) {
          ...
          if (!cancelled) {
            ...
            setOcrStatusMessage(ok ? null : "OCR is not configured. Set XAI_API_KEY on the server.");
          }
        }
      } catch {
        if (!cancelled) {
          ...
          setOcrStatusMessage("Couldn’t verify OCR status from the server.");
        }
      }
      if (!cancelled && getPlatform().ocr.supportsLiveCamera()) {
        ...
        if (denied) {
          if (!cancelled) {
            ...
            setCameraOn(false);
            setCameraError("Camera access was denied earlier. Use Add from Library.");
          }
        } else {
          try {
            ...
            if (!cancelled) {
              ...
              setCameraDeniedSticky(true);
              setCameraError("Camera permission denied. You can still add photos from your library.");
              setCameraOn(false);
            }
          }
        }
  ...
  const saveReceiptSnapshot = (allItems: DetectedItem[], imageDataUrl: string | null) => {
    ...
    const receipt = buildReceiptFromScan({
      items: allItems.map(({ name, qty, unit, emoji, storage, price, brand }) => ({
        name, qty, unit, emoji, storage, price, brand,
      })),
      imageDataUrl,
  ...
  const emitScanOutcome = (kind: "pantry" | "labels", description?: string) => {
    ...
    if (kind === "labels") {
      toast.success("Labels saved", { description: description || "Expiry notes attached" });
      return;
    }
  ...
  const startProcessing = async (photoList: CapturedPhoto[]) => {
    ...
    try {
      if (typeof navigator !== "undefined" && navigator.onLine === false) {
        toast.error("You're offline", {
          description: "Receipt reading needs a connection. Reconnect and tap Process again.",
        });
        return;
      }
    } catch {}
    ...
    try {
      ...
      for (let i = 0; i < total; i++) {
        ...
        setProcessPhase("enhance");
        setProcessLabel("Enhancing photos");
        setProcessSub(total === 1 ? "Crop · contrast · sharpen for clearer text" : `Photo ${i + 1} of ${total}`);
        setPhotoStates((prev) => {
          const next = [...prev];
          next[i] = "enhancing";
          return next;
        ...
        try {
          enhanced = await prepareImageForOcr(photoList[i].dataUrl, { enhance: true, maxEdge: 1600, quality: 0.88 });
        } catch {
        ...
        setProcessSub(total === 1 ? "Vision OCR on your photo" : `OCR running · ${readDone} of ${total} finished`);
        ...
        const job = platform.ocr.detectFromImage(enhanced).then((result) => {
          ...
          setProcessSub(total === 1 ? "Vision OCR on your photo" : `Read ${readDone} of ${total} photos`);
        });
        ocrJobs.push(job);
      }
      ...
      if (!merged.ok || merged.items.length === 0) {
        ...
        setDetected([]);
        setReviewItems([]);
        showResultThen({ ok: false, message: "Couldn't read receipt", next: "error", errorText: msg });
        return;
      }

      ocrMetaRef.current = { store: merged.store, total: merged.total, currency: merged.currency };
      ...
      const { autoItems, reviewItems: ambiguous, excludedItems } = splitAutoAndReview(results, pantryItems);
      ...
      setProcessSub(results.length === 1 ? "1 item ready" : `${results.length} items ready`);

      const skippedNonFood = excludedItems.length;
      if (skippedNonFood > 0) {
        toast.message(
          skippedNonFood === 1 ? "Skipped 1 non-pantry item" : `Skipped ${skippedNonFood} non-pantry items`,
          { description: "Cleaning, household, and personal care stay off your fridge.", duration: 3200 }
        );
      }
      ...
      if (ambiguous.length > 0) {
        ...
        showResultThen({ ok: true, message: parts.join(" · "), next: "review", summary });
      } else {
        ...
        setResultMessage(
          autoItems.length > 0
            ? [
                autoAdded > 0 ? `Added ${autoAdded} item${autoAdded === 1 ? "" : "s"}` : null,
                autoUpdated > 0 ? `Updated ${autoUpdated}` : null,
      ...
      const text = err instanceof Error ? err.message : "Something went wrong while reading the receipt.";
      setErrorMessage(text);
      showResultThen({ ok: false, message: "Processing failed", next: "error", errorText: text });
    }
  };
  ...
  const startCamera = async () => {
    ...
    if (cameraDeniedSticky) {
      setCameraError("Camera was denied earlier this session. Use Add from Library.");
      return;
    }
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
  ...
  const handleShutter = () => {
    ...
    void (async () => {
      try {
        const prepared = await captureAndPrepareFrame(video, { maxEdge: 1600, quality: 0.9, fast: true });
        if (!prepared) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (next.length === 0) {
        ...
        toast.success(next.length === 1 ? "Photo added" : `${next.length} photos added`, {
  ...
  const removeReviewItem = (id: string) => setPendingRemoveReviewId(id);
  ...
  const batchKeepNonFood = () => {
    setReviewItems((prev) =>
      prev.map((i) =>
        i.possiblyNonFood ? { ...i, possiblyNonFood: false, disposition: "add_new" as const } : i
      )
    );
  };
  const batchDiscardNonFood = () => setReviewItems((prev) => prev.filter((i) => !i.possiblyNonFood));
  const batchKeepLowConf = () => {
    ...
    toast.message("Kept low-confidence items", { description: "Confirm when ready — edit names if needed." });
  };
  const batchDiscardLowConf = () => {
    setReviewItems((prev) => prev.filter((i) => i.possiblyNonFood || i.confidence >= AUTO_ADD_CONFIDENCE));
  };

  const confirmReview = () => {
    if (reviewItems.length === 0) {
      const autoOnly = detected.filter((i) => !reviewItems.some((r) => r.id === i.id));
      if (autoOnly.length > 0) offerExpiryAssist(autoOnly, autoOnly);
      else {
    ...
    const mergeN = reviewItems.filter((i) => i.pantryMatch && (i.disposition ?? "merge") === "merge").length;
    const updateN = reviewItems.filter((i) => i.pantryMatch && i.disposition === "update").length;
      ...
      <div className="flex-1 flex flex-col bg-background rounded-t-3xl mt-auto max-h-[94dvh] overflow-hidden shadow-2xl">
        <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-border/60">
          <div className="min-w-0">
            <div className="font-semibold text-lg tracking-tight">{scanHeaderTitle(step, resultOk)}</div>
        ...
        >
          ...
          {step === "result" && (
            <ReceiptResultStage resultOk={resultOk} resultMessage={resultMessage} summary={outcomeSummary} />
          )}
          ...
          {step === "expiry-assist" && (
            <ReceiptExpiryAssistStage items={expiryCandidates} onSkip={leaveToPantry} onDone={finishExpiryAssist} />
          )}
        </div>
        ...
        {step === "review" && (
          <ReceiptReviewFooter
            ...
            onSkip={() => {
              const reviewIds = new Set(reviewItems.map((r) => r.id));
              const autoPart = detected.filter((i) => !reviewIds.has(i.id));
              if (autoPart.length > 0) offerExpiryAssist(autoPart, autoPart);
              else {
                onNavigateToPantry?.();
                handleClose();
              }
